	...
	<div class="min-h-screen bg-canvas text-ink" data-panel>
		{#if storeCode}
			...
			<div class="lg:pl-64">
				...
				{#if mobileOpen}<div class="fixed inset-0 z-50 bg-black/40 lg:hidden" role="presentation" tabindex="-1" onclick={() => (mobileOpen = false)} onkeydown={(event) => { if (event.key === 'Escape') mobileOpen = false; }}><aside class="h-full w-72 border-r border-hairline bg-card p-5" onclick={(event) => event.stopPropagation()}><div class="mb-8 flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-ember text-white"><i class="ri-store-2-line"></i></span><b>Tiendly</b><button class="ml-auto" onclick={() => (mobileOpen = false)} aria-label="Cerrar menú"><i class="ri-close-line text-xl"></i></button></div>{#each navItems as item}<a href={`/dashboard/s/${storeCode}${item.href}`} class:is-current={isActive(item.href)} class="mb-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-muted no-underline"><i class={item.icon}></i>{item.label}</a>{/each}</aside></div>{/if}
