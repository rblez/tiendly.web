<script lang="ts">
	...
	const PHYSICAL_DELIVERY_OPTIONS = [{ value: 'both', label: 'Domicilio y recogida en local' }, { value: 'delivery', label: 'Solo domicilio' }, { value: 'pickup', label: 'Solo recogida' }] as const;
	const DIGITAL_DELIVERY_OPTIONS = [{ value: 'digital', label: 'Entrega digital' }] as const;
	const MIXED_DELIVERY_OPTIONS = [{ value: 'both', label: 'Entrega física y digital' }, { value: 'physical', label: 'Solo entrega física' }, { value: 'digital', label: 'Solo entrega digital' }] as const;
	const STEP_META = [
	...
	let exchangeCash = $state('650'); let exchangeTransfer = $state('980'); let deliveryMode = $state<'pickup' | 'delivery' | 'both' | 'digital' | 'physical'>('both');
	let deliveryKind = $derived(category === 'Servicios digitales' ? 'digital' : category === 'Servicios profesionales' || category === 'Otros' ? 'mixed' : 'physical');
	let deliveryOptions = $derived(deliveryKind === 'digital' ? DIGITAL_DELIVERY_OPTIONS : deliveryKind === 'mixed' ? MIXED_DELIVERY_OPTIONS : PHYSICAL_DELIVERY_OPTIONS);
	...
	function selectCategory(value: string) { category = value; if (value === 'Servicios digitales') deliveryMode = 'digital'; else if (value === 'Servicios profesionales' || value === 'Otros') deliveryMode = 'both'; else deliveryMode = 'both'; }
</script>

<svelte:head><title>Nueva tienda | Tiendly</title></svelte:head>
<section class="max-w-lg mx-auto px-4 sm:px-6 pb-8 sm:pb-14 flex flex-col min-h-dvh">
	<div class="sticky top-0 z-30 -mx-4 bg-canvas/95 px-4 pb-5 pt-6 backdrop-blur-md sm:-mx-6 sm:px-6 sm:pt-8">
		<div class="flex justify-center"><a href="/" aria-label="Tiendly"><img src="/tiendly-logo.webp" alt="Tiendly" class="h-9 object-contain" /></a></div>
	</div>
	...
	{:else}<div class="sticky top-[6.75rem] z-20 -mx-4 bg-canvas/95 px-4 pb-5 pt-2 backdrop-blur-md sm:-mx-6 sm:px-6"><div class="mb-5"><p class="text-xs font-semibold text-ember uppercase tracking-wide mb-2">Paso {step} de 5</p><h1 class="text-2xl sm:text-3xl font-black text-ink leading-tight">{STEP_META[step - 1].title}</h1><p class="text-sm text-muted mt-2">{STEP_META[step - 1].desc}</p></div>
		<div class="flex items-center gap-2">{#each STEP_META as meta, i}<div class="flex items-center gap-2 flex-1"><span class={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold ${i < step ? 'bg-ember text-white' : 'bg-bone text-muted-soft'}`}>{i < step - 1 ? '✓' : i + 1}</span><span class="hidden sm:block text-xs font-semibold text-muted">{meta.short}</span>{#if i < 4}<div class={`flex-1 h-px ${i < step - 1 ? 'bg-ember' : 'bg-hairline'}`}></div>{/if}</div>{/each}</div></div>
		...
		{:else if step === 2}<div class="max-h-[min(55vh,30rem)] overflow-y-auto overscroll-contain rounded-card pr-1"><div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">{#each STORE_CATEGORIES as c}<button type="button" onclick={() => selectCategory(c.name)} class={`text-left px-4 py-3 rounded-btn border ${category === c.name ? 'border-ember bg-ember/10' : 'border-hairline bg-bone'}`}><span class="block text-sm font-semibold text-ink">{c.name}</span><span class="block text-xs text-muted mt-0.5">{c.desc}</span></button>{/each}</div></div>
		...
		{:else if step === 4}<div class="relative z-10 bg-card border border-hairline rounded-card p-6 space-y-5"><div class="rounded-btn bg-ember/10 border border-ember/20 p-3 text-xs text-body">La moneda base de tu tienda será <strong class="text-ink">USD</strong>. Las tasas solo convierten precios a CUP.</div><div class="grid grid-cols-2 gap-3"><label class="text-sm font-medium text-body">CUP efectivo<input bind:value={exchangeCash} type="number" min="1" class="input mt-1.5" /></label><label class="text-sm font-medium text-body">CUP transferencia<input bind:value={exchangeTransfer} type="number" min="1" class="input mt-1.5" /></label></div><p class="text-xs text-muted">Sugerencias iniciales: 650 efectivo y 980 transferencia. Revísalas antes de crear tu tienda.</p><label class="relative z-20 text-sm font-medium text-body">Modalidad de entrega<select bind:value={deliveryMode} class="input relative z-30 mt-1.5 bg-card">{#each deliveryOptions as option}<option value={option.value}>{option.label}</option>{/each}</select><span class="mt-1.5 block text-xs font-normal text-muted">{deliveryKind === 'digital' ? 'Entrega por link, archivo o código después de confirmar el pago.' : deliveryKind === 'mixed' ? 'Elige si entregarás de forma física, digital o ambas.' : 'Configura cómo recibirán tus clientes los productos físicos.'}</span></label></div>
		...
		<div class="sticky bottom-0 z-30 -mx-4 mt-6 flex items-center gap-3 border-t border-hairline bg-canvas/95 px-4 py-4 backdrop-blur-md sm:-mx-6 sm:px-6"><button type="button" disabled={step === 1} onclick={() => step -= 1} class="px-5 py-3 border border-hairline text-body rounded-btn text-sm disabled:opacity-0">Atrás</button>{#if step < 5}<button type="button" onclick={next} disabled={!canContinue} class="btn btn-3d btn-md flex-1 disabled:opacity-40">Continuar <i class="ri-arrow-right-line ml-1"></i></button>{:else}<button type="button" onclick={createStore} disabled={creating || !canContinue} class="btn btn-3d btn-md flex-1 disabled:opacity-40">{creating ? 'Creando tu tienda…' : 'Crear mi tienda'}</button>{/if}</div>
	{/if}
