...
{#if open}
	<div
		class="fixed inset-0 z-[70] flex items-center justify-center bg-ink/70 backdrop-blur-md p-4 sm:p-6"
		style="display:flex;align-items:center;justify-content:center;background-color:color-mix(in srgb, var(--color-ink) 70%, transparent);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);"
		use:bodyScrollLock
	...
	>
		<div
			class="relative w-full max-w-sm max-h-[min(32rem,calc(100vh-2rem))] overflow-y-auto bg-card/95 backdrop-blur-xl border border-hairline rounded-card shadow-2xl"
			style="margin:auto;background-color:color-mix(in srgb, var(--color-card) 95%, transparent);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);"
			role="dialog"
			aria-modal="true"
